...
export function EditUnit({ uuid, onClose }: Props) {
  ...
  const [initialValues, setInitialValues] = useState<IUnitRegisterForm | null>(
    null
  )
  ...
  async function handleOnSubmit(formValues: IUnitRegisterForm) {
    try {
      ...
      const { data, message } = await put(
        `/parametrizations/units/${formValues.uuid}`,
        formValues
      )
      ...
      if (data) {
        addToast({
          type: 'success',
          title: 'Sucesso',
          description: 'O cadastro da unidade foi editado.'
        })
